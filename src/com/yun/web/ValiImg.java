...
public class ValiImg extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for(int i=0;i<4;i++){
			...
			g.drawLine(getRNum(0,width), getRNum(0,height), getRNum(0,width), getRNum(0,height));
		}
		//6.写字符
		...
		for(int i = 0;i<4;i++){
			double ro = getRNum(-45, 45)*1.0/180*Math.PI;
			g.rotate(ro, 5+width/4*i, 20);
			g.setColor(new Color(getRNum(0,255),getRNum(0,255),getRNum(0,255)));
			String s = base.charAt(getRNum(0,base.length()-1))+"";
			buffer.append(s);
			g.drawString(s, 5+width/4*i, 20);
			g.rotate(ro*-1, 5+width/4*i, 20);
		}
	...
	private int getRNum(int begin,int end){
